...
export class SeriesHttpService {
  ...
  } // response => response.results.filter( serie => serie.poster_path )
  ...
  public getResponseOf( rating: string, page?: number ): Observable<SeriesResponse> {
    ...
    switch( rating ) {
      case 'nowPlaying'    :  return this.getAiringToday(); // últimas.
      case 'upcoming'   :  return this.getOnTheAir(); // estrenos.
